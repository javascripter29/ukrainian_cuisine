...
export const useRecipeStore = create<IRecipeState>((set) => ({
    ...
    loadRecipes: async () => {
        ...
        try {
            ...
            set({ error: 'Помилка під час завантаження рецептів', isLoading: false })
        }
    },
    addRecipe: async (formData: FormData) => {
        ...
        try {
            ...
            console.log('error', error);
            set({ error: 'Помилка під час додавання рецепту', isLoading: false })
            return { success: false, error: 'Помилка під час додавання рецепту'}
        }
    },
    updateRecipe: async (id: string, formData: FormData) => {
        ...
        try {
            ...
            console.log('error', error);
            set({ error: 'Помилка під час оновлення рецепту', isLoading: false})
            return { success: false, error: 'Помилка під час оновлення рецепту'}
        }
    },
    removeRecipe: async (id: string) => {
        ...
        try {
            ...
            console.log('error', error);
            set({ error: 'Помилка під час видалення рецепту', isLoading: false})
        }
    }
}))
